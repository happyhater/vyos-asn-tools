...
case ${FAMILY} in
	4) echo "delete policy prefix-list prefix4-${AS}";;
	6) echo "delete policy prefix-list6 prefix6-${AS}";;
esac
...
for prefix in ${PREFIXES}; do
	case ${FAMILY} in
		4)
			...
			if [ "${IMPORT}" == "ANY" ]
			then
			echo "set policy prefix-list prefix4-${AS} rule ${i} le 31"
			echo "set policy prefix-list prefix4-${AS} rule ${i} ge 1"
			fi
		;;
		6)
